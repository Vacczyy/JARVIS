...
def standby(sr, source, recognizer, user_api_key):
   print("\nStandby initiated.")
   script_active = False
   ...
   while time.time() - start_time < timeout:
      ...
      try:
         recorded = recognizer.recognize_whisper_api(audio, api_key = user_api_key)
   ...
   if script_active:
      while not end_conversation:
         end_conversation = listening(sr, source, recognizer, user_api_key, end_conversation)
   else:
      exit()
   return True

def listening(sr, source, recognizer, user_api_key, end_conversation):
   print("Listening.")
   start_time = time.time()
   while time.time() - start_time < timeout:
      audio = recognizer.listen(source)
      try:
         transcription = recognizer.recognize_whisper_api(audio, api_key=user_api_key)
         if transcription:
            inscribe(transcription)
            print("Audio transcribed.")
            end_conversation = text_classification(transcription, user_api_key, end_conversation)
            break
      except sr.UnknownValueError:
